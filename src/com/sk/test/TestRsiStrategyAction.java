...
import com.sk.strategy.LowestRsiStrategy;
import com.sk.util.MathUtil;

import java.io.File;
import java.util.Date;

public class TestRsiStrategyAction {
			...
			for(DayStock dayStock :stock.getDayStockList()) {
				int dayStockIndex = dayStock.getIndex();
			 	if(!dayStock.getDate().startsWith("2014-11-24")){
			  		...
			  		boolean isNewHigh = new LowestRsiStrategy().conformLowestRsiStrategy(stock, dayStock);
			  		if(checkAll){
			  			isNewHigh = true;
			  		}
				 		if( isNewHigh ) {
